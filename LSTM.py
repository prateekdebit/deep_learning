    ...
    with tf.device("/cpu:0"):
      ...
      for l in f:
        ...
        if len(strings)==301:
          ...
          count+=1
      f.close()
      #print('suceesfull begginnig')
      #print(word_dict['inclined'])      
      #print('succesfull ending')
      #embedding = tf.get_variable("embedding", [vocab_size, size])
      #inputs = tf.nn.embedding_lookup(embedding, self._input_data)
      matrix = np.vstack(tuple(vectors))
      W1 = tf.Variable(matrix,name="W1")
      #print("runned till here1")
      #W = tf.Constant(w2v,name="W"); 
...
def run_epoch(session, m, data, eval_op,str2, verbose=False):
  #print('__________no error______________')
  #print(m.batch_size,       m.num_steps)
  ...
  #print(state)
  #print('__________no error______________')
  #print(m.batch_size,       m.num_steps,       len(data))
...
'''
def run_epoch2(session, m, data, eval_op, verbose=False):
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    with tf.variable_scope("model", reuse=True, initializer=initializer):
      ...
      mtest1 = PTBModel(is_training=False, config=eval_config)
      mtest2 = PTBModel(is_training=False, config=eval_config)
      mtest3 = PTBModel(is_training=False, config=eval_config)
      mtest4 = PTBModel(is_training=False, config=eval_config)
    ...
    for i in range(config.max_max_epoch):
      ...
      #valid_perplexity = run_epoch(session, mvalid, valid_data, tf.no_op(),'train')
    		...
    		with open("MSR_Sentence_Completion_Challenge_V1/Data/questions.txt") as f:
    			...
    			for line in f:
    				...
    				if i%5==0 :
    					...
    					#print(test_perplexity2)
    					#print(str(test_perplexity2)+'  '+str(low))
